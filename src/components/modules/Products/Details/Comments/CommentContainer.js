...
function CommentContainer() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-10 max-w-full max-sm:overflow-x-hidden">
      ...
      <div className="order-1 md:order-2 col-span-12 xl:col-span-4 md:col-span-5 w-full md:h-[500px] md:sticky md:top-20">
        <PostComment />
      </div>

      {/* ستون چپ کامنت‌ها */}
      <div className="order-2 md:order-1 col-span-12 xl:col-span-8 md:col-span-7">
        <h2 className="text-xl text-gray-800 dark:text-gray-300 mb-6">نظرات کاربران</h2>

        {/* فقط کامنت‌ها divide داشته باشن */}
        <div className="divide-y-2 divide-gray-200 dark:divide-slate-700">
          <Comment />
          <Comment />
          <Comment />
          <Comment />
