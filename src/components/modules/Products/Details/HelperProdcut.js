...
export default function HelperProduct() {
  ...
  return (
    <div dir="rtl" className="w-full my-10  font-medium">
      {/* تب‌ها */}
      <div className="relative border-b-2 border-gray-200 dark:border-slate-700">
          ...
          {tabs.map((tab, index) => (
            <button
              ...
              className={`
                ...
                ${
                  activeTab === tab.id
                    ? "text-green-600 dark:text-slate-400 font-bold shadow-lg bg-white dark:bg-slate-600/20 -mb-0.5 z-10"
                    : "text-gray-500 hover:text-gray-800 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-400/20"
                }
              `}
            >
              {tab.title}
        ...
        <span
          className="max-sm:hidden absolute -bottom-0.5 h-1 z-10 bg-green-600 dark:bg-slate-400 rounded-full transition-all duration-500 ease-out shadow-md"
          style={{
      ...
      {/* محتوای تب */}
      <div className="mt-8 p-8 max-md:px-4 bg-white dark:bg-slate-600/20 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700">
        <div className="text-gray-700 dark:text-gray-300 leading-9 text-lg">
          {contents[activeTab]}
        </div>
      </div>
    </div>
